...
def main(args):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        toch_prediction = toch_prediction.cpu()
        pred = toch_prediction.detach().numpy()
        break
    train_input = train_input.numpy()
    train_Gtruth = train_Gtruth.cpu()
    train_Gtruth = train_Gtruth.numpy()
    np.save("predictions/input.npy",train_input)
    ...
    test_Gtruth = test_Gtruth.cpu()
    test_Gtruth = test_Gtruth.numpy()
    np.save("predictions/test_input.npy",test_input)
    for i in range(args.batch_size):
